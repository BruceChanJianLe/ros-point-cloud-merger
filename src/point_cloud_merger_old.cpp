    ...
    point_cloud_merger::point_cloud_merger() : private_nh_("~"), global_nh_(), tf2_listener_(tfBuffer)
    {
        ...
        for (int i = 0; i < MAX_SIZE; i++)
        {
            ...
            if (i >= input_size)
            {
                ...
                /* boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[i](new message_filters::Subscriber<PointCloudMsgT>(global_nh_, store_input_topics[i], QUEUE_SIZE)); */
            ...
            else
            {
                ...
                /* boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[i](new message_filters::Subscriber<PointCloudMsgT>(global_nh_, store_input_topics[0], QUEUE_SIZE)); */
        ...
        /* boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers(new message_filters::Subscriber<PointCloudMsgT>[MAX_SIZE]); */
        /* boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_(new message_filters::Subscriber<PointCloudMsgT>[MAX_SIZE], [](message_filters::Subscriber<PointCloudMsgT>* p) { delete[] p;});

        message_filters::Subscriber<PointCloudMsgT>[1]-> */
        ...
        /* boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[0] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[0], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[1] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[1], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[2] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[2], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[3] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[3], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[4] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[4], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[5] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[5], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[6] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[6], QUEUE_SIZE);
        boost::shared_ptr<message_filters::Subscriber<PointCloudMsgT>> cloud_subscribers_[7] = boost::make_shared<message_filters::Subscriber<PointCloudMsgT>>(global_nh_, store_input_topics[7], QUEUE_SIZE); */

        /* pointcloud not showing */
        /* HERE */
